...
export class UploadService {
  ...
  async preprocessMP3(fileBuffer: Buffer): Promise<Buffer> {
    ...
    await new Promise<void>((resolve, reject) => {
      exec(command, (error, stdout, stderr) => {
        if (error) {
          console.error(
            'Erreur lors du nettoyage du fichier MP3 avec FFmpeg:',
            stderr,
          );
          reject(error);
          return;
  ...
  async generateHLS(
    fileBuffer: Buffer,
  ): Promise<{ segments: Buffer[]; manifest: Buffer }> {
    ...
    try {
      ...
      if (!manifestContent.includes('#EXTINF')) {
        throw new Error(
          `Manifest invalide : aucun segment trouvé. Contenu : ${manifestContent}`,
        );
      }
      ...
      if (!targetDurationMatch || parseInt(targetDurationMatch[1], 10) <= 0) {
        throw new Error(
          `Manifest invalide : durée cible incorrecte. Contenu : ${manifestContent}`,
        );
      }
   ...
   * @param relativePath Le chemin relatif pour accéder aux segments (par défaut: `/api/music/stream/segment/`).
   * @returns Le contenu du manifest mis à jour avec les chemins relatifs des segments.
   */
  rewriteHLSManifestWithRelativePath(
    manifest: string,
    segmentCIDs: string[],
    relativePath: string = '/api/music/stream/segment/',
  ): string {
  ...
  async uploadHLSFilesToIPFS(
    ...
    const manifestCID = await this.storageService.uploadToIPFS(
      Buffer.from(updatedManifest, 'utf-8'),
    );
